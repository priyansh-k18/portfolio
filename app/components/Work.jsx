      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 my-10 gap-5'>
       {workData.map((project, index) => (
        ...
        style={{backgroundImage:`url(${project.bgImage})`}}>
          <div className={`${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'} w-10/12 rounded-md absolute bottom-5
          left-1/2 -translate-x-1/2 py-3 px-5 flex items-center
          justify-between duration-500 group-hover:bottom-7 transition-colors duration-200`}>
            <div>
               <h2 className='font-semibold'>{project.title}</h2>
               <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{project.description}</p>
            </div>
            <div className={`border rounded-full ${isDarkMode ? 'border-white' : 'border-black'} w-9
